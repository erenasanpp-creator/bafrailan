...
export default function Panel() {
  ...
  async function login(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!prof || prof.role !== "admin") { await supabase.auth.signOut(); setErr("Bu hesap admin değil."); return; }
    setMe(prof);
  }
  ...
  if (!me || me.role !== "admin") {
    return (
      <div className="max-w-md mx-auto mt-16 p-6 rounded-xl border bg-white">
        <h1 className="text-xl font-semibold mb-3">Yönetim Girişi</h1>
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={logout} className="px-3 py-1.5 rounded-md border">Çıkış</button>
      </div>
      {/* Panel içeriklerini buraya ekle */}
    </div>
  );
}
